...
package ippoz.reload.algorithm.utils;
...
public abstract class KdTree<T> {
    ...
    public static class ELKIHell<T> extends KdTree<T> {
        ...
        protected double pointDist2(double[] p1, double[] p2) {
            ...
            double agg = 0.;
            for(int d = 0; d < mindim; d++) {
              final double v = Math.sqrt(p1[d]) - Math.sqrt(p2[d]);
              agg += v * v;
            }
     ...
     */
    public static class ELKIEuclid<T> extends KdTree<T> {
        ...
        protected double pointDist(double[] p1, double[] p2) {
        	double sum = 0.0;
            for(int i=0;i<p1.length;i++) {
               sum = sum + Math.pow((p1[i]-p2[i]),2.0);
            }
    ...
    public static class SqrEuclid<T> extends KdTree<T> {
        public SqrEuclid(Integer sizeLimit) {
            super(Integer.MAX_VALUE, sizeLimit);
        }
